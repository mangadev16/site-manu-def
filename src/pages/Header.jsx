...
import {
  ...
  PlusCircle, ClipboardList, PhoneCall, X, UserCircle2, Home, LogIn
} from "lucide-react";
  ...
  return (
    <>
      ...
      <aside className={`lg:hidden fixed top-0 left-0 h-full w-64 bg-white z-[60] shadow-2xl transform transition-transform duration-300 ${menuAberto ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        {/* Botão de login/logout no drawer */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-100">
          {user ? (
            <button
              onClick={() => { auth.signOut(); setMenuAberto(false); }}
              className="w-full flex items-center gap-3 p-4 rounded-xl text-red-600 font-bold hover:bg-red-50 transition-colors"
            >
              <LogOut size={18} /> Sair
            </button>
          ) : (
            <button
              ...
              className="w-full flex items-center gap-3 p-4 rounded-xl text-emerald-700 font-bold bg-emerald-50 hover:bg-emerald-100 transition-colors"
            >
          ...
          <div className="relative" style={{ overflow: "visible" }}>
            {user ? (
              <>
                ...
                >
                  <User size={15} />
                  <span className="text-xs font-bold hidden sm:inline">{nomeUsuario || "Perfil"}</span>
                ...
                {perfilAberto && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-50">
                    ...
                    {/* Sair só aparece no desktop — no mobile está no drawer */}
                    <button
                      onClick={() => auth.signOut()}
                      className="hidden lg:flex w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 items-center gap-2 font-bold border-t border-gray-50"
                    >
                      <LogOut size={16} /> Sair
                    </button>
                  </div>
              ...
              <button
                ...
                className="flex items-center gap-2 bg-white/15 border border-white/25 px-3 py-2 rounded-xl hover:bg-white/25 transition-all text-xs font-bold"
              >
                <LogIn size={15} />
