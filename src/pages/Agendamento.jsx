...
import {
  ...
  Activity, PlusCircle, ClipboardList, RotateCcw, Settings, LogOut, PhoneCall
} from "lucide-react";
...
const Agendamento = () => {
  ...
  const finalizarAgendamento = async () => {
    try {
      ...
      const q = query(collection(db, "agendamentos"), where("data", "==", dataString), where("horario", "==", horarioSelecionado));
      const querySnapshot = await getDocs(q);
        ...
        <div className="w-full max-w-md bg-white rounded-[35px] shadow-xl border border-emerald-50 overflow-hidden">
          {etapa === 1 && (
            <div className="p-6">
              ...
              <button onClick={()=>setEtapa(2)} className="w-full mt-4 bg-emerald-600 text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-emerald-700 active:scale-95">Ver Horários</button>
            </div>
          )}

          {etapa === 2 && (
            <div className="p-6">
              <button onClick={()=>setEtapa(1)} className="flex items-center gap-2 text-emerald-600 font-bold mb-4 text-sm"><ArrowLeft size={16} /> Voltar ao calendário</button>
              <div className="bg-emerald-50 p-4 rounded-2xl text-center mb-6 border border-emerald-100"><p className="text-emerald-800 font-bold">{dataSelecionada.toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long"})}</p></div>
              <h3 className="font-bold text-gray-800 mb-4">Horários disponíveis:</h3>
              {horariosDisponiveis.length>0?(
                <div className="grid grid-cols-3 gap-3">
                  {horariosDisponiveis.map(hora=>{
                    const ocupado=horariosOcupados.includes(hora);
                    return <button key={hora} disabled={ocupado} onClick={()=>{setHorarioSelecionado(hora);setEtapa(3);}} className={`p-4 rounded-2xl font-bold border-2 transition-all ${ocupado?"bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed opacity-60":horarioSelecionado===hora?"bg-emerald-600 border-emerald-600 text-white shadow-lg":"bg-white border-emerald-50 text-gray-700 hover:border-emerald-200"}`}>{ocupado?"Ocupado":hora}</button>;
                  })}
                </div>
          ...
          {etapa === 3 && (
            <div className="p-6 space-y-3">
              ...
              <div className="bg-emerald-50 p-4 rounded-2xl text-center mb-4 border border-emerald-100"><p className="text-gray-700 font-bold">{dataSelecionada.toLocaleDateString("pt-BR")} às {horarioSelecionado}</p></div>
              {[{id:"Nutrição",nome:"Nutrição",icon:<Activity className="text-emerald-600"/>,cor:"bg-emerald-50"},{id:"Acupuntura",nome:"Acupuntura",icon:<PenLine className="text-blue-600"/>,cor:"bg-blue-50"},{id:"Farmácia",nome:"Farmácia",icon:<ClipboardList className="text-purple-600"/>,cor:"bg-purple-50"}].map(s=>(
                <button key={s.id} onClick={()=>{setServicoSelecionado(s.nome);setEtapa(4);}} className={`w-full p-5 rounded-2xl border-2 border-gray-100 flex items-center justify-between hover:border-emerald-500 transition-all ${s.cor}`}>
                  <div className="flex items-center gap-4"><div className="bg-white p-2 rounded-lg shadow-sm">{s.icon}</div><span className="font-bold text-gray-700">{s.nome}</span></div><ChevronRight size={20} className="text-gray-400" />
                </button>
              ))}
            </div>
          )}
          ...
          {etapa === 5 && (
            <div className="p-6 text-center py-10 space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              <div className="bg-white border border-emerald-100 rounded-3xl p-6 shadow-sm"><p className="text-emerald-800 font-bold text-lg">{servicoSelecionado}</p><p className="text-gray-500">{dataSelecionada.toLocaleDateString("pt-BR")} às {horarioSelecionado}</p></div>
