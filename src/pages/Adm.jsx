...
// 2. Crie um projeto → Storage → New bucket → nome: "pdfs" → Public bucket ✓

// 3. Settings → API → copie os dois valores abaixo:

const SUPABASE_URL = "https://ahyjwxyiluoesrbjkxpu.supabase.co";      

const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFoeWp3eHlpbHVvZXNyYmpreHB1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI3NjcyMTIsImV4cCI6MjA5ODM0MzIxMn0.g4bDIVbqdPjlhWUIyeoDRPdKP2u3SnP4FKzKvl-isio";    
...
const ModalidadeBadge = ({ modalidade }) => {
  ...
  return (
    ...
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold border ${
      ...
      online ? "bg-sky-50 text-sky-600 border-sky-100" : "bg-emerald-50 text-emerald-700 border-emerald-100"

    }`}>

      {online ? <Wifi size={10} /> : <MapPin size={10} />}
  ...
  const enviarDocumentacao = async () => {
    ...
    try {
      ...
      if (arquivoPDF) {
        ...
        setUploadProgresso(30); // Supabase não expõe progresso granular via fetch
        ...
        const caminho = `documentos/${nomeUnico}`;



        const respUpload = await fetch(

          `${SUPABASE_URL}/storage/v1/object/${SUPABASE_BUCKET}/${caminho}`,

          {
            ...
            headers: {
              ...
              "Content-Type": arquivoPDF.type || "application/pdf",
        ...
        if (!respUpload.ok) {
          ...
          const erro = await respUpload.text();
        ...
        // URL pública do arquivo (bucket público)

        urlPDF = `${SUPABASE_URL}/storage/v1/object/public/${SUPABASE_BUCKET}/${caminho}`;
                          ...
                          <tr key={index} onClick={() => abrirModalDocumentacao(ag)} className="hover:bg-slate-50/50 transition group text-xs cursor-pointer" title="Clique para emitir documentação">
                            ...
                            <td className="py-4 px-6">
                              ...
                              <div className="flex flex-col gap-1 items-start">
                                ...
                                <ModalidadeBadge modalidade={ag.modalidade} />
                  ...
                  {uploadStatus === "enviando" && (
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between text-[10px] text-slate-400 font-bold">
                        ...
                        <span className="flex items-center gap-1.5"><Loader2 size={12} className="animate-spin" style={{ color: ROXO_DESTAQUE }} /> Enviando PDF...</span>
