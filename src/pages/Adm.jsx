    ...
    <div className="fixed inset-0 h-screen w-full bg-gray-50 font-sans flex flex-col overflow-hidden">
      ...
      <header className="bg-[#1f2937] text-white p-4 flex justify-between items-center shadow-md z-50 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-sm uppercase leading-none tracking-tight">Painel de Controle</h1>
            <p className="text-[10px] uppercase opacity-90">Gestão de Agendamentos</p>
          </div>
        </div>

        <div className="relative">
          <button onClick={() => setPerfilAberto(!perfilAberto)} className="flex items-center gap-2 bg-gray-800 px-3 py-2 rounded-full border border-gray-600">
            <User size={16} />
          ...
          {perfilAberto && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-50">
              <button onClick={() => auth.signOut()} className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 font-bold transition-colors">
      ...
      <main className="flex-1 p-4 lg:p-10 flex flex-col items-center overflow-y-auto">
        <div className="w-full max-w-[800px] space-y-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Próximos atendimentos</h2>
              ...
              {agendamentosTodos.map((ag) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-800 text-lg leading-tight">{ag.usuarioNome}</h3>
                      <p className="text-emerald-600 font-bold text-sm uppercase tracking-wider">{ag.servico}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-emerald-600 p-6 text-white flex justify-between items-center">
              <h3 className="font-bold text-lg uppercase">Detalhes do Agendamento</h3>
              <button onClick={() => setAgendamentoSelecionado(null)} className="hover:bg-emerald-700 p-1 rounded-full"><X size={24} /></button>
            </div>
            
            <div className="p-8 space-y-6">
              <div className="space-y-1">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Paciente</span>
                <p className="text-xl font-bold text-gray-800">{agendamentoSelecionado.usuarioNome}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Data</span>
                  <div className="flex items-center gap-2 font-bold text-gray-700"><Calendar size={16}/> {agendamentoSelecionado.data}</div>
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Horário</span>
                  <div className="flex items-center gap-2 font-bold text-gray-700"><Clock size={16}/> {agendamentoSelecionado.horario}</div>
                </div>
              </div>

              <div className="p-4 bg-emerald-50 rounded-2xl border-l-4 border-emerald-500">
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Serviço Solicitado</span>
                <p className="text-lg font-extrabold text-emerald-900">{agendamentoSelecionado.servico}</p>
              </div>
